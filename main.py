...
class Sistemainvestimento:
    def __init__(self, root):
        ...
        # As outras telas serão configuradas dinamicamente
    ...
    def configurar_tela_listar_semanas(self):
        # MUDANÇA: Tela de listagem genérica
        tela = self.telas["tela_listar_semanas"]
        for widget in tela.winfo_children(): widget.destroy()
        imovel = self.imovel_selecionado
        
        frame = tk.Frame(tela, pady=20, bg="#f0f0f0")
        frame.pack(fill="both", expand=True)
        tk.Label(frame, text=f"Semanas Disponíveis - {imovel.nome}", font=("Arial", 16, "bold")).pack()
        
        colunas = ('semana_id', 'numero', 'periodo', 'valor', 'status')
        self.tabela_semanas = ttk.Treeview(frame, columns=colunas, show='headings', height=15)
        self.tabela_semanas.heading('semana_id', text='ID')
        self.tabela_semanas.heading('numero', text='Semana Nº')
        self.tabela_semanas.heading('periodo', text='Período')
        self.tabela_semanas.heading('valor', text='Valor (R$)')
        self.tabela_semanas.heading('status', text='Status')
        self.tabela_semanas.column('semana_id', width=0, stretch=tk.NO) # Oculta a coluna de ID
        
        semanas_db = db.buscar_semanas_por_imovel(imovel.id)
        for semana in semanas_db:
            status = "Disponível" if semana['proprietario_id'] is None else f"Ocupada por {semana['dono_nome']}"
            self.tabela_semanas.insert('', 'end', values=(semana['id'], semana['numero'], semana['periodo'], f"{semana['preco']:.2f}", status))
        
        self.tabela_semanas.pack(pady=10, padx=20, fill="both", expand=True)
        
        tk.Button(frame, text="Comprar Semana Selecionada", command=self.comprar_semana_selecionada).pack(pady=10)
        tk.Button(frame, text="Voltar", command=lambda: self.mostrar_tela("tela_descricao_imovel", imovel=imovel)).pack()
        
    def comprar_semana_selecionada(self):
        selecionado = self.tabela_semanas.selection()
        if not selecionado:
